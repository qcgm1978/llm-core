...
// 创建可修改的提示模板对象
let prompts = JSON.parse(JSON.stringify(defaultPrompts));
// 获取所有语言选项
export const getLanguages = () => {
    return prompts.languages;
};
// 获取指定语言的所有提示模板
export const getPromptsByLanguage = (language) => {
    return prompts[language] || [];
};
// 根据名称和语言获取提示模板
export const getPromptByName = (name, language = 'zh') => {
    const promptsForLang = prompts[language];
    if (!promptsForLang)
        return undefined;
    const prompt = promptsForLang.find(p => p.act === name);
    if (!prompt) {
        return promptsForLang.find(p => p.act === 'wiki')?.prompt;
    }
    return prompt?.prompt;
};
// 替换提示模板中的变量
export const formatPrompt = (prompt, replacements) => {
    let result = prompt;
...
// 添加或更新提示模板
export const updatePrompt = (name, prompt, language = 'zh') => {
    const promptsForLang = prompts[language];
    if (!promptsForLang)
        return false;
