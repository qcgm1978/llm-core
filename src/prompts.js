...
var defaultPrompts = {
    ...
    zh: [
        ...
        {
            act: '纯文本',
            prompt: '"{topic}"。不要使用markdown、标题或任何特殊格式。只返回定义本身的文本。'
        },
        {
            act: 'wiki',
        ...
        {
            act: '思维导图箭头生成',
            prompt: '请根据给定的思维导图数据，生成节点之间的箭头连接关系。箭头表示因果关系、依赖关系、时间顺序或逻辑关系。\n\n要求：\n1. 使用JSON格式输出，只包含箭头信息\n2. 每个箭头对象包含以下字段：\n   - from: 起始节点ID\n   - to: 目标节点ID\n   - label: 箭头标签（描述关系的简短文本）\n3. 不要返回任何JSON以外的内容\n4. 不要复制思维导图数据，只返回箭头数组\n5. 箭头数组应命名为"arrows"'
        }
    ],
    en: [
        // 默认提示模板
        {
            act: 'Concise Definition',
        ...
        },
        {
            ...
            prompt: 'Please generate arrow connections between nodes based on the given mind map data. Arrows represent causal relationships, dependencies, chronological order, or logical relationships.\n\nRequirements:\n1. Output in JSON format, containing only arrow information\n2. Each arrow object contains the following fields:\n   - from: starting node ID\n   - to: target node ID\n   - label: arrow label (short text describing the relationship)\n3. Do not return anything other than JSON\n4. Do not copy the mind map data, only return the arrow array\n5. The arrow array should be named "arrows"'
        }
    ]
};
// 创建可修改的提示模板对象
var prompts = JSON.parse(JSON.stringify(defaultPrompts));
// 获取所有语言选项
export var getLanguages = function () {
    return prompts.languages;
};
// 获取指定语言的所有提示模板
export var getPromptsByLanguage = function (language) {
    return prompts[language] || [];
};
// 根据名称和语言获取提示模板
export var getPromptByName = function (name, language) {
    var _a;
    if (language === void 0) { language = 'zh'; }
    var promptsForLang = prompts[language];
    if (!promptsForLang)
        return undefined;
    var prompt = promptsForLang.find(function (p) { return p.act === name; });
    if (!prompt) {
        return (_a = promptsForLang.find(function (p) { return p.act === 'wiki'; })) === null || _a === void 0 ? void 0 : _a.prompt;
    }
    return prompt === null || prompt === void 0 ? void 0 : prompt.prompt;
};
// 替换提示模板中的变量
export var formatPrompt = function (prompt, replacements) {
    var result = prompt;
...
// 添加或更新提示模板
export var updatePrompt = function (name, prompt, language) {
    if (language === void 0) { language = 'zh'; }
    var promptsForLang = prompts[language];
    if (!promptsForLang)
        return false;
    var index = promptsForLang.findIndex(function (p) { return p.act === name; });
    if (index >= 0) {
        promptsForLang[index].prompt = prompt;
    }
    else {
        promptsForLang.push({ act: name, prompt: prompt });
    }
    return true;
};
// 重置所有提示模板到默认值
export var resetPrompts = function () {
